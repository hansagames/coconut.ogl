...
class Attributes {
	static public function isDisplayObject(c:ClassType)
		return switch c {
			case { pack: ['ogl'], name: 'Transform' }: true;
			default: false;
		}

	static public function attrType(f)
		return
			if (f.isDisplayObject) macro : coconut.ogl.RenderResult;
	...
	static function build() {
		return tink.macro.BuildCache.getType("coconut.ogl.internal.Attributes", null, null, function(ctx) {
			var name = ctx.name,
			...
			if (!isDisplayObject(cls)) {
				...
				attr = TIntersection([macro : coconut.ogl.internal.Attributes<$parent>, attr]);
			}
			...
			function add(f, type:ComplexType)
				attrs.push({
					...
					kind: FProp('default', 'never', if (f.name == dFault && type.match(macro : String)) macro : coconut.ogl.Text else type),
					access: [AFinal],
				});
